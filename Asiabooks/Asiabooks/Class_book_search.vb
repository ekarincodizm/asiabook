...
Public Class Class_book_search
    ...
    Function _dt_book_search_Internet()
        ...
        strsql &= " , '0' as ebook_format , search.keyword , '1' as indexs "

        strsql &= " FROM "

        strsql &= " (select top(1000) book.isbn_13 , book.book_title , UPPER(book.book_title) as book_title_search "
        ...
        strsql &= " , cast(book.link_isbn as varchar) as other_format , book.stock_status "
        ...
        strsql &= " order by book.stock_status DESC ) as search "
        ...
        strsql &= " FROM "
        strsql &= " (select distinct top(200) cast(ebook.isbn_13 as varchar) as isbn_13 , ebook.bookid as ebook_id , cast(ebook.on_book as varchar) as other_format "
        strsql &= " from ebook_store ebook with (nolock) "
        strsql &= " where convert(numeric(13,2) , ebook.price_org) > 0 and ebook.status = 'active' and ebook.price_org is not null "
        ...
        Dim datasort As DataTable = datatable
        datatable = New DataTable
        datasort.DefaultView.Sort = "indexs,book_title asc"
        datatable = datasort.DefaultView.Table()
    ...
    Function ebook()
        ...
        strsql &= " (select distinct top(200) cast(ebook.isbn_13 as varchar) as isbn_13 , ebook.bookid as ebook_id , cast(ebook.on_book as varchar) as other_format "
        strsql &= " from ebook_store ebook with (nolock) "
        strsql &= " where ebook.status = 'active' and ebook.price_org is not null "
        strsql &= " " + equal_ + " "
        If country = "TH" Then
            strsql &= " and ebook.th = 1 "
            strsql &= " ) as search "
        ...
        strsql &= " FROM "
        strsql &= " (select distinct top(500) cast(ebook.isbn_13 as varchar) as isbn_13 , ebook.bookid as ebook_id , cast(ebook.on_book as varchar) as other_format "
        strsql &= " from ebook_store ebook with (nolock) "
        strsql &= " where ebook.status = 'active' and ebook.price_org is not null "
        If by = "keyword" Then
            strsql &= " " + like_ + " "
    ...
    Function book()
        ...
        strsql &= " (select top(200) book.isbn_13 , book.book_title , UPPER(book.book_title) as book_title_search "
        strsql &= " , case book.author when '' then 'NONE' else book.author end as author , UPPER(book.author) as author_search "
        strsql &= " , case book.publisher_name when '' then '-' else book.publisher_name end as publisher_name "
        ...
        strsql &= " and convert(numeric(13,2), book.selling_price) > 0 and book.field3 = '' "
        strsql &= " " + equal_ + " "
        strsql &= " order by book.stock_status DESC , book.selling_price ASC ) as search "
        ...
        strsql &= " , search.author_search , search.publisher_name , search.image , search.source "
        strsql &= " , cast(search.ecom_discount as varchar) as ecom_discount , 'book' as book_type "
        strsql &= " , '0' as ebook_format , search.keyword , '2' as indexs "

        strsql &= " FROM "

        strsql &= " (select top(500) book.isbn_13 , book.book_title , UPPER(book.book_title) as book_title_search "
        strsql &= " , case book.author when '' then 'NONE' else book.author end as author , UPPER(book.author) as author_search "
        strsql &= " , case book.publisher_name when '' then '-' else book.publisher_name end as publisher_name "
        strsql &= " , convert(numeric(18,2),ecom_discount) as ecom_discount , book.source "
        strsql &= " , convert(numeric(13,2), book.selling_price) as selling , book.image "
    ...
    Function book_and_ebook()
        ...
        strsql &= " SELECT DISTINCT TOP(200) "
        strsql &= " ' ' as selling_price , ' ' as selling_price_ecomn "
        ...
        strsql &= " FROM "
        strsql &= " (select distinct top(200) cast(ebook.isbn_13 as varchar) as isbn_13 , ebook.bookid as ebook_id "
        ...
        strsql &= " , search.author_search , search.publisher_name , search.image , search.source "
        strsql &= " , cast(search.ecom_discount as varchar) as ecom_discount , 'book' as book_type "
        strsql &= " , '0' as ebook_format , search.keyword , '3' as indexs "
        ...
        strsql &= " , search.isbn_13 , '' as book_title , '' as book_title_search , '' as author , '' as author_search "
        strsql &= " , '' as publisher_name , '' as image , '' as source , '' as ecom_discount , 'ebook' as book_type "
        strsql &= " , '' as ebook_format , '' as keyword , '4' as indexs "

        strsql &= " FROM "
        strsql &= " (select distinct top(200) cast(ebook.isbn_13 as varchar) as isbn_13 , ebook.bookid as ebook_id "
        strsql &= " , cast(ebook.on_book as varchar) as other_format , ebook.book_cuttitle "
        strsql &= " from ebook_store ebook with (nolock) "
        strsql &= " where ebook.status = 'active' and ebook.price_org is not null and ebook.price_org is not null "
        If by = "keyword" Then
            strsql &= " " + like_ + " "
        Else
            strsql &= " " + like_ + " "
        ...
        'Dim datasort As DataTable = datatable
        'datatable = New DataTable
        'datasort.DefaultView.Sort = "indexs asc"
        'datatable = datasort.DefaultView.Table()
        Return datatable
    End Function
    ...
    Function get_format_ebook()
        ...
        strsql &= " else ROUND(convert(numeric(18,2),(search.selling * isnull(search.exchange_rate,0))),0) "
        ...
        strsql &= " , convert(numeric(13,2) , ebook.price_org) as selling , ebook.isbn_10 as image , cast(ebook.on_book as varchar) as other_format "
        ...
        End Try
        sql_datatable.DefaultView.Sort = "other_format desc"
    ...
    Function author_search()
        ...
        strsql &= " FROM "
        strsql &= " (select top(200) cast(ebook.isbn_13 as varchar) as isbn_13 , ebook.bookid as ebook_id "
        strsql &= " , cast(ebook.on_book as varchar) as other_format , ebook.book_cuttitle "
        strsql &= " from ebook_store ebook with (nolock) "
        strsql &= " where ebook.status = 'active' and ebook.price_org is not null "

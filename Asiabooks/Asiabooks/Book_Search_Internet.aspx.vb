...
Partial Class Book_Search_Internet
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
    End Sub
    Private Sub SaveUserAcess()
        Dim counter As Integer = 0
        Dim strSql As String = ""
        Dim dt_count As New DataTable
        Dim strKeyWord As String = Request.QueryString("keyword").ToString
        uCon = New uConnect

        strSql = ""
        strSql &= " select isnull(max(Counter),0)as Counter from dbo.BSA_KeywordCount"
        strSql &= " where [Type] = 'All' and [Keyword] = '" + strKeyWord.ToString.Replace("'", "'+Char(39)+'") + "' and SearchBy = 'IN'"
        dt_count.Clear()
        dt_count = uFunction.getDataTable(uCon.conn, strSql)

        If dt_count.Rows.Count > 0 Then
            If dt_count.Rows(0).Item("Counter").ToString = 0 Then
                counter = counter + 1
                Dim strInsert As String = ""
                strInsert = ""
                strInsert &= " INSERT INTO BSA_KeywordCount([Type],[Keyword],[SearchBy],[Counter],[Updated],[Result]) VALUES ("
                strInsert &= " 'All'"
                strInsert &= " ,'" + strKeyWord.ToString.Replace("'", "'+Char(39)+'") + "'"
                strInsert &= " ,'IN'"
                strInsert &= " ,'" + counter.ToString + "'"
                strInsert &= " ,getdate() "
                strInsert &= " ,'" + lblResult.Text + "')"
                uFunction.ExecuteDataStringNonTran(uCon.conn, strInsert)
            Else
                counter = Val(dt_count.Rows(0).Item("Counter").ToString) + 1
                Dim strUpdate As String = ""
                strUpdate = ""
                strUpdate &= " Update BSA_KeywordCount set"
                strUpdate &= " [Counter] = '" + counter.ToString + "',[Result] = '" + lblResult.Text + "',[Updated] = getdate() "
                strUpdate &= " where [Type] = 'All' and [Keyword] = '" + strKeyWord + "' and SearchBy = 'IN'"
                uFunction.ExecuteDataStringNonTran(uCon.conn, strUpdate)
            End If
        End If
    ...
    Private Sub binddata()
        ...
        Try
            datatable = class_search._dt_book_search_Internet
        Catch ex As Exception
            Response.Redirect("BooksNot_Found.aspx?Meassge=Data not found")
        End Try

        Dim qxx = (From a In datatable.AsEnumerable() _
                Where a.Field(Of String)("Book_Title_Search") Like "" + Request.QueryString("keyword").ToString.ToUpper + "*" _
                Select a).Union( _
                From b In datatable.AsEnumerable() _
                Where b.Field(Of String)("ISBN_13") Like "" + Request.QueryString("keyword").ToString + "*" _
                Select b).Union( _
                From c In datatable.AsEnumerable() _
                Where c.Field(Of String)("Author_Search") Like "" + Request.QueryString("keyword").ToString.ToUpper + "*" _
                Select c).Union( _
                From d In datatable.AsEnumerable() _
                Where d.Field(Of String)("keyword") Like "*" + Request.QueryString("keyword").ToString.ToUpper + "*" _
                Select d)

        If datatable.Rows.Count > 0 Then
            If qxx.Count > 0 Then
                datatable = New DataTable
                datatable = qxx.CopyToDataTable
            End If
            ...
            lblResult.Text = datatable.Rows.Count.ToString
            SaveUserAcess()
            If datatable.Rows.Count = 1 Then
                If Left(datatable.Rows(0).Item("isbn_13").ToString, 3) = "MAG" Then
                    Response.Redirect("Magazine_Detail_Internet.aspx?Title_1=" + datatable.Rows(0).Item("ISBN_13").ToString + "")
                Else
                    Response.Redirect("book_detail_Internet.aspx?Title_1=" + datatable.Rows(0).Item("ISBN_13").ToString + "&Book_Type=" + datatable.Rows(0).Item("Book_Type").ToString + "")
                End If
            End If
        Else
            Response.Redirect("BooksNot_Found.aspx?Meassge=Data not found")
    ...
    Protected Sub Datagrid_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.DataGridItemEventArgs) Handles Datagrid.ItemDataBound
        Try
            ...
            If e.Item.ItemType = ListItemType.AlternatingItem Or e.Item.ItemType = ListItemType.Item Then
                ...
                '///// show available /////
                If IsNumeric(Session("keyword").ToString) Then
                ...
                price_usd = class_book_detail.callUsd(price) ' convert bath to USD
                lbl_price_usd.Text = "US$ " + Convert.ToDouble(price_usd).ToString("#,###.00")
    ...
    Protected Sub Datagrid_PageIndexChanged(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridPageChangedEventArgs) Handles Datagrid.PageIndexChanged
        ...
        'Else
        'Me.Datagrid.CurrentPageIndex = e.NewPageIndex
        'Me.Datagrid.DataSource = Session("datatable")
        'Me.Datagrid.DataBind()
        ''binddata()
        'End If
    ...
    Protected Sub Datagrid_DeleteCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles Datagrid.DeleteCommand
        Dim datatable As DataTable = Session("datatable")

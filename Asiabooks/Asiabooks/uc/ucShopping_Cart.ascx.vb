...
Partial Class uc_ucShopping_Cart
    ...
    Private Sub BindData()
        ...
        Dim count_item_ebook As Double = 0

        If Session("In_Branch") Is Nothing And Session("jobber") Is Nothing And Session("ebook_shopping") Is Nothing Then
        ...
        Else
            ...
            If Not IsNothing(Session("ebook_shopping")) Then
                Dim dt_ebook As New DataTable
                dt_ebook = Session("ebook_shopping")
                If dt_ebook.Rows.Count > 0 Then
                    For i As Integer = 0 To dt_ebook.Rows.Count - 1
                        count_amount = CDbl(dt_ebook.Rows(i).Item("Items_Amount")) + count_amount
                        count_qty = CDbl(dt_ebook.Rows(i).Item("Quantity")) + count_qty
                    Next
                    count_item_ebook = dt_ebook.Rows.Count
                End If
            End If


            lbl_amount.Text = count_amount.ToString("#,###,##0.00")
            lbl_item.Text = CDbl(count_item_branch.ToString) + CDbl(count_item_jobber.ToString) + CDbl(count_item_ebook.ToString)

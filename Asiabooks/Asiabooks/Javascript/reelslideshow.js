...
function reelslideshow(options){
	...
	this.setting.longestdesc="" //get longest description of all slides. If no desciptions defined, variable contains ""
	...
	for (var i=0; i<setting.imagearray.length; i++){ //preload images
		...
		if (setting.longestdesc.length > 0) {
		    slidesHTML += reelslideshow.routines.getSlideTextHTML(setting.imagearray[i], setting.dimensions[0] + 'px', setting.dimensions[1] + 'px', this.posprop) + '\n'
		} else {
...
reelslideshow.routines={
	...
	getSlideHTML:function(imgref, w, h, posprop){
		...
		layerHTML += '<img src="' + imgref[0] + '" style="border-width:0;" width="' + w + '" height="' + h + '"/>'
	...
	getSlideTextHTML: function(imgref, w, h, posprop) {
	    var posstr = posprop + ":" + ((posprop == "left") ? w : h)
	    var layerHTML = (imgref[1]) ? '<a href="' + imgref[1] + '" target="' + imgref[2] + '" style="text-decoration:none;font-family: Arial, Helvetica, sans-serif;font-size: 12px;color: #666666;">' : '' //hyperlink slide?
	    layerHTML += imgref[3]
	    layerHTML += (imgref[1]) ? '</a>' : ''
	    return '<div class="slide" style="position:absolute;' + posstr + ';width:' + w + ';height:' + h + ';text-align:center;">'
			+ '<div Class="labelprice" style="width:' + w + ';height:' + h + ';display:table-cell;vertical-align:middle;">'
			+ layerHTML
			+ '</div></div>' //return HTML for this layer
	},

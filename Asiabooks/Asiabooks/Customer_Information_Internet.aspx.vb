...
Partial Class Customer_Information_Internet
    ...
    Private Sub setDefault()
        tbx_email.Attributes.Add("onkeydown", "if(event.which || event.keyCode){if ((event.which == 13) || (event.keyCode == 13)) {document.getElementById('" + b_search.UniqueID + "').click();return false;}} else {return true}; ")
        tbx_pass.Attributes.Add("onkeydown", "if(event.which || event.keyCode){if ((event.which == 13) || (event.keyCode == 13)) {document.getElementById('" + b_search.UniqueID + "').click();return false;}} else {return true}; ")

        tbx_name.Attributes.Add("onkeydown", "if(event.which || event.keyCode){if ((event.which == 13) || (event.keyCode == 13)) {document.getElementById('" + b_add.UniqueID + "').click();return false;}} else {return true}; ")
        tbx_surname.Attributes.Add("onkeydown", "if(event.which || event.keyCode){if ((event.which == 13) || (event.keyCode == 13)) {document.getElementById('" + b_add.UniqueID + "').click();return false;}} else {return true}; ")
        tbx_email1.Attributes.Add("onkeydown", "if(event.which || event.keyCode){if ((event.which == 13) || (event.keyCode == 13)) {document.getElementById('" + b_add.UniqueID + "').click();return false;}} else {return true}; ")
        tbx_pass1.Attributes.Add("onkeydown", "if(event.which || event.keyCode){if ((event.which == 13) || (event.keyCode == 13)) {document.getElementById('" + b_add.UniqueID + "').click();return false;}} else {return true}; ")
        tbx_contpass.Attributes.Add("onkeydown", "if(event.which || event.keyCode){if ((event.which == 13) || (event.keyCode == 13)) {document.getElementById('" + b_add.UniqueID + "').click();return false;}} else {return true}; ")


        Me.hdSessiongRegister.Value = "captcharegister"
    ...
    Protected Sub b_search_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles b_search.Click
        ...
        If Me.tbx_pass.Text = "" Then
            uScript.Alert(Me.Page, Me.GetType, "Please fill in your password.")
            tbx_pass.Focus()
            Exit Sub
        End If
        ...
        Try
            ...
            'If rdtemail.Checked = True Then
            '    ds = member.GetMember(email_pass, 1, "ubKPwB6lD2fihnqJycW/TBHP5S9MYMJJ", tbx_pass.Text.Trim)
            'Else
            '    ds = member.GetMember(email_pass, 0, "ubKPwB6lD2fihnqJycW/TBHP5S9MYMJJ", tbx_pass.Text.Trim)
            'End If
            ...
            If dt_bs_person.Rows.Count > 0 Then
                ...
                If "INTERNET" = ConfigurationSettings.AppSettings("UserInternet") Then
                    Response.Redirect("Customer_Order_Internet.aspx")

                    'ScriptManager.RegisterClientScriptBlock(Me.Page, Me.GetType, "Information", _
                    '"alert('Login is successful. Welcome to the Customer Order system.');" & _
            ...
            Else
                ...
                '    "alert('Login is unsuccessful because your password is incorrect. Please try again.');", True)
                '    Exit Sub
                'Else
                '    ScriptManager.RegisterClientScriptBlock(Me.Page, Me.GetType, "Alert", _
                '    "alert('Login is unsuccessful because your Member ID is incorrect. Please try again.');", True)
                '    Exit Sub
                'End If
            End If
        Catch ex As Exception
